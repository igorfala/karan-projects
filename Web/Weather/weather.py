...
def weather(location = None):
    ...
    title = None
    for i in range(len(weatherTags)):
        if weatherTags[i].has_attr('class') and weatherTags[i].attrs['class'][0]=='headerText':
            title = weatherTags[i].string    #title is the city name as showed on the website
            break
    if title == None: # If the title is not found, the data was not fetched
